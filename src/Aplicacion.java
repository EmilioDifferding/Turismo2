...
//
//
		ResultSet rs = Basedatos.getResults("SELECT * FROM usuario");
		while(rs.next()){
//			System.out.println(rs.getString("presupuesto"));
			int usuario_id = rs.getInt("usuario_id");
			String usuario_nombre = rs.getString("nombre");
			int preferencia =  rs.getInt("atraccion_favorita");
			int presupuesto = rs.getInt("presupuesto");
			double tiempo = rs.getDouble("tiempo_disponible");

			ResultSet rsSugerencias = Basedatos.getResults("SELECT * FROM atraccion WHERE atraccion.tipo_id = "+preferencia);
			while(rsSugerencias.next()){
				//compras?

				System.out.println(rsSugerencias.getString("nombre"));
			}
